...
class WxValidate {
  ...
  formatTplAndAdd(param, rule, value) {
    ...
    const newErrorList = this.errorList
    const listLen = this.errorList.length
    let noErrorOfParam = true
    if (listLen) {
      for (let i = 0; i < listLen; i++) {
        if (this.errorList[i].param === param) {
          newErrorList[i] = error
          noErrorOfParam = false
        }
        break
      }
    }
    if (noErrorOfParam || !listLen) {
